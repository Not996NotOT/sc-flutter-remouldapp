...
import 'package:notarization_station_app/components/awesome_bottom_bar/widgets/hexagon/hexagon.dart';
...
class _InspiredState extends State<Inspired> with TickerProviderStateMixin {
  ...
  Widget buildItem(BuildContext context, { TabItem item,  int index, bool active = false}) {
    Color itemColor() {
      if (widget.fixed) {
      ...
      child: Column(
        ...
        children: [
          ...
          if (item.title is String && item.title != '') ...[
            ...
            Text(
              item.title,
              style: Theme.of(context).textTheme.subtitle2.merge(widget.titleStyle).copyWith(color: itemColor()),
