...
//引入防抖
                          ...
                          setState(() {
                            ...
                            if (_points == null || _points.isEmpty) {
                              ...
                              Future.delayed(Duration.zero, () async {
                                ...
                                print(
                                    "_imageLocalPath1-------$_imageLocalPath1");
  ...
  Widget _createGridViewItem(Color color, String _imageLocalPath) {
    ...
    // return Image(image: FileImageEx(File(_imageLocalPath ?? '')));
  }
  ...
  faceComparison(String path) async {
    ...
    try {
      ...
      print(
          '签名-----------multipartFile大小--------------${multipartFile.length}');
      offSet = qmsheet.getString('offSet');
      print('签名-----------offSet--------------$offSet');
      HomeApi.getSingleton().uploadPictures(multipartFile, errorCallBack: (e) {
        ...
        if (res['code'] == 200) {
          ...
          print(
              '签名-----------res[item][filePath]-------------${res['item']['filePath'].toString()}');
          SystemChrome.setPreferredOrientations(
...
// =========================================================================
  ...
  Future<String> _capturePng1(File toFile) async {
    ...
    imageData = pngBytes;
    await toFile.writeAsBytes(pngBytes);
    log("写入后文件的大小-----${await toFile.length()}");
    return toFile.path;
  }
}
...
class FileImageEx extends FileImage {
  int fileSize;
  FileImageEx(File file, {double scale = 3.0})
      : assert(file != null),
        assert(scale != null),
